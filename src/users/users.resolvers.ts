...
import { Args, Mutation, Query, Resolver, Context, ResolveProperty, Parent } from '@nestjs/graphql';
import { UsersService } from './users.service';
import { JwtAuthGuard } from '../auth/guards/jwt-auth.guard';
import { CreateUserInput, User, UpdateUserInput, Trip } from '../graphql.classes';
...
export class UserResolver {
  constructor(private usersService: UsersService, private tripService: TripService) {}
  ...
  @ResolveProperty()
  async trips(@Parent() user: User): Promise<TripDocument[] | undefined> {
    const { _id } = user
    return this.tripService.findAll({ ownedBy: _id})
  }
}
